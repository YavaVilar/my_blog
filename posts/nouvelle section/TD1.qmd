...
Le Stade Tour Eiffel se situe au barycentre de l'ensemble des sites olympiques. Nous pouvons obtenir une visualisation graphique de sa localisation avec le code qui suit:

```{r}
library(leaflet)

leaflet(data = site_proche_barycentre) |>
  addTiles() |>
  addMarkers(~long, ~lat)
```
